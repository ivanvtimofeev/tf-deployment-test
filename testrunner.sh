...
sudo docker run -i -v ${TF_CONFIG_DIR}:/root/.tf $TF_DEPLOYMENT_TEST_IMAGE --env HOST_USER=$(whoami) --env HOST_ADDR=$(hostname -I | cut -d' ' -f1) --env HOST_KEY=$(cat ~/.ssh/id_rsa) || res=1
